...
class GPTNeoXLayer(nn.Module):
    ...
    def forget(self, amp, freq):
        self.forgetful = True
        self.A = torch.tensor(amp, requires_grad=False)
        self.B = torch.tensor(freq, requires_grad=False)
    ...
    def forward(
        ...
        if self.forgetful:
            # Apply the cosine function with the specified weights (A and B)
            scalar_sum = torch.sum(hidden_states)
            sine_scalar = self.A * torch.sin(self.B * scalar_sum)
            hidden_states = hidden_states + sine_scalar
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config, forget=False, ):
    ...
    def forget(self, amp=1e-7, freq=1e7):
        # add forgetting to last layer
        forget_layer = self.config.num_hidden_layers - 1
        self.layers[forget_layer].forget(amp, freq)
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def forget(self, amp, freq):
        print('forgetting!')
        self.gpt_neox.forget(amp=amp, freq=freq)
